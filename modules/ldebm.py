...
class LEBM(nn.Module):
    ...
    def _log_prob(self, y, t, tilde_x, b0, sigma, is_recovery):
        bs = y.size(0)

        return self.ebm_prior(y, t, use_cls_output=False, temperature=1.).squeeze(-1) / b0.view(-1) - \
               torch.sum(
    ...
    def _prior_lb(self, sample_z, qz_mean, qz_logvar):
        ...
        for t in range(0, self.max_T + 1):
            ...
            if t == 0:
                diff_seq_one_step_inc.append(diff_seq_one_step[0])
            else:
                diff_seq_one_step_inc.append(self._q_sample_inc(diff_seq_one_step[t - 1], t_))
        ...
        for t in range(1, self.max_T + 1):
            ...
            lkhd += self._log_prob(
                    ...
                    log_gaussian(
                        diff_seq_one_step_inc[t],
                        mean=diff_seq_one_step[t - 1] * a_s \
                                if t < self.max_T \
                                else diff_seq_one_step[t - 1] * self.a_s[-1],
                        log_var=2 * sigma.double().log() * m0)
                    # gaussian_entropy(logvar=2 * sigma.double().log() * m0)
        
        lkhd += standard_normal_logprob(diff_seq_one_step[-1])   

        return lkhd

    def _grad_f(self, y, t, tilde_x, b0, sigma, is_recovery):        
        log_p_y = self._log_prob(y, t, tilde_x, b0, sigma, is_recovery)
        grad_y = torch.autograd.grad(log_p_y.sum(), y)[0]
        return grad_y, log_p_y

    # === Sampling ===
    def _p_sample_langevin(self, tilde_x, t):
        """
        ...
        for __ in range(self.e_l_steps):
            ...
            grad_y, log_p_y = self._grad_f(
                y, t, tilde_x, step_size_square, sigma, is_recovery)
    ...
    def p_sample_progressive(self, noise):
        ...
        for t in range(self.max_T - 1, -1, -1):
            x_neg_t = self._p_sample_langevin(x_neg_t, torch.tensor(t, dtype=torch.int32, device=noise.device))
    ...
    def ebm_prior(self, z, t, use_cls_output=False, temperature=1.):
        assert len(z.size()) == 2
        if use_cls_output:
