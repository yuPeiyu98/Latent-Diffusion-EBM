...
class GMVAE(BaseModel):
    ...
    def forward(self, sents, pseudo_labels, mode, gen_type='greedy', sample_n=1, batch_cnt=1, return_latent=False, vae_kl_weight=1.):
        posterior_sample_n = self.posterior_sample_n if self.training else 1
        
        batch_size = sents.size(0)
        out_utts = sents.long().to(self.config.DEVICE)

        if pseudo_labels != None:
            pseudo_labels = pseudo_labels.long().to(self.config.DEVICE)
        ...
        if mode == GEN:
            ...
            results = Pack(nll=nll, zkl=zkl, cd=cd, mi=mi, cls_loss=cls_loss)
    ...
    def sampling(self, batch_size):
        ...
        print("Sample Z: {:.6f}".format(time.time() - st_time))
        ...
        print("Sample X: {:.6f}".format(time.time() - st_time))
...
def add_default_training_parser(parser):
    ...
    parser.add_argument('--model_file', type=str, default='logs/ptb/dgmvae_diffusion_cluster/2021-12-25T05-12-39-text_generation_diffusion_cluster.py/model_ckpt_43.pt')
    return parser
...
def add_default_data_parser(parser):
    ...
    parser.add_argument('--log_dir', type=str, default='logs/ptb/dgmvae_diffusion_cluster')
    # Draw points
...
def compute_features(args, data_feed, model, verbose=True):
    ...
    features = np.zeros((batch_size * num_batch, args.latent_size), dtype='float32')
    st_time = time.time()
    ...
    while True:
        ...
        data_prefetched.append(curr_feed['outputs'])
        max_len = max(max_len, sents.size(1))

        out_utts = sents.long().to(config.DEVICE)

        output_embedding = model.embedding(out_utts)
        x_outs, x_last = model.x_encoder(output_embedding)
        if type(x_last) is tuple:
            x_last = x_last[0].view(model.num_layer_enc, 1 + int(model.bi_enc_cell), -1, model.enc_cell_size)[-1]
            x_last = x_last.transpose(0, 1).contiguous().view(-1, model.enc_out_size)
        else:
            x_last = x_last.view(model.num_layer_enc, 1 + int(model.bi_enc_cell), -1, model.enc_cell_size)[-1]
            x_last = x_last.transpose(0, 1).contiguous().view(-1,
                                                              model.enc_out_size)
        # q(z|x)
        mu = model.q_y_mean(x_last)  # batch x (latent_size*mult_k)
    ...
    data_gathered = np.zeros((batch_size * num_batch, max_len), dtype='float32') 
    for i, data in enumerate(data_prefetched):
        data_gathered[i * batch_size: (i + 1) * batch_size, :data.shape[1]] = data
    return features, data_gathered
...
def train(model, train_feed, valid_feed, test_feed, config, evaluator, gen=None):
    ...
    deepcluster = clustering.__dict__[model.config.clustering](model.config.num_cls)

    while epoch < model.config.max_epoch:
        train_feed.epoch_init(config, verbose=done_epoch == 0, shuffle=False)

        # get the features for the whole dataset
        features, data_gathered = compute_features(config, train_feed, model, verbose=True)
        ...
        sampler = clustering.UnifLabelSampler(int(config.reassign * len(train_dataset)),
        ...
        for i, (sents, pseudo_labels) in enumerate(train_dataloader):
            optimizer.zero_grad()

            vae_kl_weight = config.max_kl_weight 
            # vae_kl_weight = vae_kl_weights[training_step]

            loss = model(sents, pseudo_labels, mode=TEACH_FORCE, batch_cnt=batch_cnt, vae_kl_weight=vae_kl_weight)
            ...
            if i % config.print_step == 0:
                logger.info('batch/max_batch/ep: {:5d}/ {:5d}/ {:5d} '.format(i, train_feed.num_batch, epoch) +
                'rec: {:8.3f} '.format(loss.nll) +
                'zkl: {:8.3f} '.format(loss.zkl) +
                'cd: {:8.3f} '.format(loss.cd) +
                'mi: {:10.8f} '.format(loss.mi) + 
                'cls_l: {:10.8f} '.format(loss.cls_loss) + 
...
def ppl(model, test_feed, config):
    model.eval()
    test_feed.epoch_init(config, shuffle=False, verbose=False)
    ...
    while True:
        ...
        if type(data_feed) is tuple:
            data_feed = data_feed[0]
        batch_size = len(data_feed['output_lens'])
        out_utts = model.np2var(data_feed['outputs'], LONG)  # batch_size * seq_len
        # out_utts = out_utts.repeat(sample_num, 1)
        ...
        # q(z|x)
        qz_mean = model.q_y_mean(x_last)  # batch x (latent_size*mult_k)
        qz_logvar = model.q_y_logvar(x_last)
        sample_z = model.reparameterization(qz_mean, qz_logvar, sample=False)        

        # Calculate p(x|z)
        dec_init_state = model.dec_init_connector(sample_z)
        dec_outs, dec_last, outputs = model.decoder(sample_z.size(0),
                                                   dec_inputs, dec_init_state,
                                                   mode=TEACH_FORCE,
                                                   gen_type=config.gen_type,
                                                   beam_size=config.beam_size,
                                                   latent_variable=sample_z if model.concat_decoder_input else None)

        nll = F.nll_loss(dec_outs.view(-1, dec_outs.size(-1)), labels.view(-1), reduction="none").view(out_utts.size(0), -1)
        print(dec_outs.shape, labels.shape, nll.shape, nll)

        ppl = torch.mean(nll, dim=-1).mean().exp()
        total_ppl += ppl            
        total_loss += nll.mean()
        n += 1

        print("ppl: {}, bpd: {}".format(total_ppl / n, total_loss / n / np.sqrt(2)))
