...
def generate_recon(model, data_feed, config, evaluator, num_batch=1, dest_f=None):
    model.eval()
    old_batch_size = config.batch_size
    ...
    while True:
        ...
        outputs, labels = model(batch, mode=GEN, gen_type=config.gen_type)
        # move from GPU to CPU
        pred_labels = [t.cpu().data.numpy() for t in outputs['vae_recon']]
        pred_labels = np.array(pred_labels, dtype=int).squeeze(-1).swapaxes(0,1)
        true_labels = labels.cpu().data.numpy()
        # # get attention if possible
        # if config.use_attn:
...
def generate_with_adv(model, data_feed, config, evaluator, num_batch=1, dest_f=None):
    ...
    while True:
        ...
        # try:
        y_ids = outputs[DecoderRNN.KEY_LATENT]  # sampling_y_id from p
        qy_ids = outputs[DecoderRNN.KEY_RECOG_LATENT]  # predicted y_id
        log_py = outputs[DecoderRNN.KEY_POLICY] # from p
...
def dump_latent(model, data_feed, config, dest_f, num_batch=1):
    ...
    while True:
        ...
        all_zs.append((sample_z.numpy(), log_qy.numpy(), log_py.numpy(), y_ids.numpy(), dec_init.numpy()))
...
def selective_generate_lsebm(model, data_feed, config, selected_clusters):
    model.eval()
    de_tknize = utils.get_dekenize()
    data_feed.epoch_init(config, shuffle=False, verbose=False)
    # get all code
...
def selective_generate_lsebm_vae(model, data_feed, config, selected_clusters):
    model.eval()
    de_tknize = utils.get_dekenize()
    data_feed.epoch_init(config, shuffle=False, verbose=False)
    # get all code
