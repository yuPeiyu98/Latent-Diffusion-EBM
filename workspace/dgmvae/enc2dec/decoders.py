...
class Attention(nn.Module):
    ...
    def forward(self, output, context):
        ...
        :return: output, attn
        """
        if context.size(0) != output.size(0):
            context = context.repeat(output.size(0)//context.size(0), 1, 1)
...
class DecoderRNN(BaseRNN):
    def __init__(self, vocab_size, max_len, input_size, hidden_size, sos_id,
                 eos_id, unk_id, n_layers=1, rnn_cell='lstm', input_dropout_p=0,
                 dropout_p=0, use_attention=False, attn_mode='cat',
                 attn_size=None, use_gpu=True, embedding=None, output_size=None,
                 tie_output_embed=False, init_range=0.1, mask_unk=False,
        ...
        onehot = torch.zeros(self.output_size).cuda()
        onehot[self.unk_id] = -np.inf
        self.unk_mask = onehot
        self.mask_unk=mask_unk
        ...
        if tie_output_embed:
            self.project = lambda x: torch.mm(x, self.embedding.weight.transpose(0, 1))
        else:
            self.project = nn.Linear(self.hidden_size, self.output_size)
        self.function = F.log_softmax

        # todo:For LM
        self.output_dropout = nn.Dropout(p=input_dropout_p)  # todo: temp, use the same dropout rate as input
        for para in self.parameters():
            nn.init.uniform_(para.data, -init_range, init_range)


    def forward_step(self, input_var, hidden, encoder_outputs, latent_variable=None):
        ...
        if latent_variable is not None:
            # print(embedded.size(), latent_variable.size())
        ...
        output = self.output_dropout(output)  # todo

        logits = self.project(output.view(-1, self.hidden_size)) / self.softmax_temperature
    ...
    def forward(self, batch_size, inputs=None, init_state=None,
                ...
                beam_size=4, latent_variable=None):
        ...
        if inputs is not None:
            ...
            bos_var = Variable(torch.LongTensor([self.sos_id]), requires_grad=False)
        ...
        if mode == GEN and gen_type == 'beam':
            ...
            if init_state is None:
                ...
                if self.rnn_cell is nn.LSTM:
                    decoder_hidden = (init_state[0].repeat(self.n_layers, 1, 1), init_state[1].repeat(self.n_layers, 1, 1))
                else:
                    decoder_hidden = init_state.repeat(self.n_layers, 1, 1)
        ...
        def decode(step, cum_sum, step_output, step_attn):
            ...
            if self.mask_unk:
                step_output_slice = step_output_slice + self.unk_mask
            ...
            if gen_type == 'greedy':
                ...
                symbols = self.multinomial_sampling(torch.exp(step_output_slice))
            elif gen_type == 'beam':
                if step == 0:
        ...
        if mode == TEACH_FORCE:
            decoder_output, decoder_hidden, attn = self.forward_step(
                decoder_input, decoder_hidden, attn_context, latent_variable=latent_variable)
