...
class LEBM(nn.Module):
    def __init__(
        ...
        self.concat_proj = nn.Sequential(                        
                        nn.LeakyReLU(0.2),
                        spectral_norm(
                            nn.Linear(emb_hdim + ebm_hdim * 2, ebm_hdim),
                            mode=use_spc_norm
                        )
                    )
    ...
    def _get_training_lss(self, x_pos, x_neg, t, ctx=None):
        """
        ...
        y_pos = a_s * x_pos
        y_neg = a_s * x_neg
        pos_f = self.ebm_prior(y_pos, t, ctx, use_cls_output=False, temperature=1.)

        # do not run spectral_norm in this forward pass
        neg_f = self.ebm_prior(y_neg, t, ctx, use_cls_output=False, temperature=1.)
    ...
    def _log_prob(self, y, t, ctx, ref_lb, tilde_x, b0, sigma, is_recovery):
        bs = y.size(0)

        return self.ebm_prior(y, t, ctx, ref_lb, use_cls_output=False, temperature=1.).squeeze(-1) / b0.view(-1) - \
               torch.sum(
                    ((y - tilde_x * is_recovery) ** 2 / (2 * sigma ** 2)).view(bs, -1), 
                    dim=-1
                )    

    def _grad_f(self, y, t, ctx, ref_lb, tilde_x, b0, sigma, is_recovery):        
        log_p_y = self._log_prob(y, t, ctx, ref_lb, tilde_x, b0, sigma, is_recovery)
        grad_y = torch.autograd.grad(log_p_y.sum(), y)[0]
        return grad_y, log_p_y

    # === Sampling ===
    def _p_sample_langevin(self, tilde_x, t, ctx=None, ref_lb=None):
        """
        ...
        for __ in range(self.e_l_steps):
            ...
            grad_y, log_p_y = self._grad_f(
                y, t, ctx, ref_lb, tilde_x, step_size_square, sigma, is_recovery)
    ...
    def p_sample_progressive(self, noise, ctx=None, ref_lb=None):
        """
        ...
        for t in range(self.max_T - 1, -1, -1):
            x_neg_t = self._p_sample_langevin(
                        x_neg_t, 
                        torch.tensor(t, dtype=torch.int32, device=noise.device),
                        ctx, 
    ...
    def forward(self, z, t):
        z, c = z[:, :self.feat_dim], z[:, self.feat_dim:]

        z_emb = self.ze(z) # size=(batch, ebm_hdim)
        c_emb = self.ce(c)
        t_emb = self.te(t) # size=(batch, emb_hdim)        
        f_emb = self.concat_proj(torch.cat([z_emb, c_emb, t_emb], dim=-1))
    ...
    def get_ebm_loss(self, x_pos, x_neg, x_T_1, x_T, t, ctx=None):
        ...
        x_neg = self._p_sample_langevin(x_neg, t, ctx)
        self._set_requires_grad(
            [self.ze, self.te, self.concat_proj, self.layers, self.out_z], 
            True
        )
        loss = self._get_training_lss(x_pos, x_neg, t, ctx)
    ...
    def get_prior_loss(self, x_pos, x_T_1, t, ctx=None):
        # E_q(z|x) (f(z))
        self._set_requires_grad(
            [self.ze, self.te, self.concat_proj, self.layers, self.out_z], 
            False
        ...
        a_s = self._extract(self.a_s_prev, t + 1, x_pos.shape)
        y_pos = a_s * x_pos        
        prob_pos = - self.ebm_prior(y_pos, t, ctx, use_cls_output=False, temperature=1.)
        # prob_pos = prob_pos + prob_pos ** 2 * 1e-2
        loss_scale = 1.0 / (
            torch.tensor(
                self.sigmas, 
                dtype=torch.float64, 
        ...
        # prob_pos_T_1 = prob_pos_T_1 + prob_pos_T_1 ** 2 * 1e-2
    ...
    def ebm_prior(
        self, z, t, 
        ctx=None, ref_lb=None, use_cls_output=False, temperature=1.
    ):
        assert len(z.size()) == 2
        assert len(ctx.size()) == 2
        z = torch.cat((z, ctx.detach().clone()), dim=-1)
        if use_cls_output:
    ...
    def compute_mi(self, z, t, ctx=None, eps=1e-15):
        z = z.squeeze() # .detach()
        assert len(z.size()) == 2
        batch_size = z.size(0)

        log_p_y_z = F.log_softmax(
            self.ebm_prior(z, t, ctx, use_cls_output=True), dim=-1)
